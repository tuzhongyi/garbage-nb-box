...
interface ScanCodeVideoControllerEvent {
  data(imageData: ImageData): void
  url(url: string): void
  zoom(value: number): void
}

export class ScanCodeVideoController {
  event = new EventEmitter<ScanCodeVideoControllerEvent>()
  constructor() {
    this.regist()
  }
  private element = {
  ...
  private tick() {
    ...
    this.event.emit('url', this.element.canvas.toDataURL())
  ...
  test() {
    ...
    img.onload = () => {
      ...
      ctx.drawImage(
        ...
        0,
        0,
        this.element.parent.clientWidth,
        this.element.parent.clientHeight
      )
      this.event.emit('url', this.element.canvas.toDataURL())
    }
  }
